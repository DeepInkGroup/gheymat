...
- [`src/lib/symbols.ts`](src/lib/symbols.ts) — metadata for all 32 tracked instruments (name, category, unit, icon/flag, brand color).
- [`src/lib/goldApi.ts`](src/lib/goldApi.ts) — server-side fetch from [gold-api.com](https://gold-api.com/docs) for Silver (XAG), Gold spot (XAU), Copper (HG) and Palladium (XPD); free, unauthenticated, no rate limit. Same self-measured-delta approach as baha24.ts. Merged with baha24's items via [`src/lib/allPrices.ts`](src/lib/allPrices.ts). All four start hidden — opt in from settings.
